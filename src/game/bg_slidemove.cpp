...
qboolean PM_SlideMove(qboolean gravity) {
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    for (i = 0; i < numplanes; i++) {
      if (DotProduct(trace.plane.normal, planes[i]) > 0.99) {
        if (extrabumps <= 0) {
          ...
          // zinx - if it happens again,
          // nudge the origin instead,
          // and trace it, to make sure
          // we don't end up in a solid
    ...
    for (i = 0; i < numplanes; i++) {
      ...
      if (into >= 0.1) {
        continue; // move doesn't interact with
                  // the plane
      }
      ...
      // see if there is a second plane that the new
      // move enters
      for (j = 0; j < numplanes; j++) {
        ...
        // see if it goes back into the first
        // clip plane
        if (DotProduct(clipVelocity, planes[i]) >= 0) {
          continue;
        }

        // slide the original velocity along the
        // crease
        ...
        // see if there is a third plane the the
        // new move enters
      ...
      // if we have fixed all interactions, try another
      // move
...
void PM_StepSlideMove(qboolean gravity) {
  ...
  if (pm->debugLevel) {
    ...
    if (trace.allsolid && !wassolid) {
      Com_Printf("%i:PM_SlideMove solidified! (%f %f "
                 "%f) -> (%f %f %f)\n",
    ...
    if (PM_SlideMove(gravity) == 0) {
      return; // we got exactly where we wanted to go
              // first try
    }
  }
