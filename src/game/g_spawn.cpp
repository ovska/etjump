...
namespace ETJump{
	...
	static void initPhasing()
	{
		...
		const int onDeath = PHASEOPT_RESETONDEATH;
		const int onLoad = PHASEOPT_RESETONLOAD;

		G_Printf(
			"Phasing is %s on death and %s on load/goto.\n",
			(level.phaseOptions & onDeath) ? "reset" : "preserved",
			(level.phaseOptions & onLoad) ? "reset" : "preserved");
	}
}
