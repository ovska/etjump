...
const int TARGETPHASE_A_ON  = 0x01;
const int TARGETPHASE_A_OFF = 0x02;
const int TARGETPHASE_A_TGL = 0x04;
const int TARGETPHASE_B_ON  = 0x08;
const int TARGETPHASE_B_OFF = 0x10;
const int TARGETPHASE_B_TGL = 0x20;
...
void SP_target_phase(gentity_t* self)
{
	...
	if (G_SpawnString("phasea", "", &phasea))
	{
		...
		{
			Com_Printf("Invalid phase A setting, defaulting to none: ^3%s\n", phasea);
		}
	}
	...
	if (G_SpawnString("phaseb", "", &phaseb))
	{
		...
		{
			Com_Printf("Invalid phase B setting, defaulting to none: ^3%s\n", phaseb);
		}
	}

	if (!flags)
	{
		G_DPrintf("Error: Nonfunctional target_phase defined");
	}
	else
	{
		self->count = flags;
